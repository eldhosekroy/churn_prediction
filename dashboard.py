...
def preprocess(candidate_profile, call_log, executive_profile):
    ...
    # ── Executive Features ───────────────────────
    exec_feat = executive_profile[['Executive_ID', 'Experience_Years', 'Team', 'Location']].copy()
    exec_feat.rename(columns={'Experience_Years':'Executive_Experience',
                               'Team':'Executive_Team',
                               'Location':'Executive_Location'}, inplace=True)
    ...
    exec_agg = call_with_exec.groupby('Candidate_ID').agg(
        ...
        Executive_Location   = ('Executive_Location',   lambda x: x.mode()[0] if len(x.mode()) > 0 else 'Unknown'),
    ).reset_index()
...
def page_live_predictor(df, model_data):
    ...
    with cc1:
        total_calls     = st.number_input("Total Calls", 0, 20, 2, key="p_tc")
        unique_execs    = st.number_input("Unique Executives", 0, 10, 1, key="p_ue")
    with cc2:
        total_call_dur  = st.number_input("Total Call Duration (min)", 0.0, 120.0, 8.0, step=0.5, key="p_tcd")
        avg_call_dur    = st.number_input("Avg Call Duration (min)",   0.0,  60.0, 4.0, step=0.5, key="p_acd")
    with cc3:
        max_call_dur    = st.number_input("Max Call Duration (min)",   0.0,  60.0, 6.0, step=0.5, key="p_mxcd")
        min_call_dur    = st.number_input("Min Call Duration (min)",   0.0,  30.0, 2.0, step=0.5, key="p_mncd")
    with cc4:
        call_freq       = st.number_input("Call Frequency (per month)", 0.0, 20.0, 2.0, step=0.5, key="p_cf")
        exec_exp        = st.number_input("Avg Executive Experience (yrs)", 0.0, 15.0, 5.0, step=0.5, key="p_ee")
    ...
    exec_loc  = st.selectbox("Executive Location", sorted(df['Executive_Location'].dropna().unique()) if 'Executive_Location' in df.columns else ['Delhi','Mumbai'], key="p_el")
...
def page_model_performance(df, model_data):
    ...
    with i1:
        st.markdown("""<div class="kpi-card"><div class="kpi-icon">🤖</div>
            ...
            <div class="kpi-value" style="color:#a78bfa; font-size:16px; margin-top:8px;">Random Forest</div>
            <div class="kpi-sub">Regularized</div></div>""", unsafe_allow_html=True)
    with i2:
        st.markdown(f"""<div class="kpi-card"><div class="kpi-icon">🌳</div>
            <div class="kpi-title">Estimators</div>
            <div class="kpi-value" style="color:#60a5fa;">{getattr(model,'n_estimators','100')}</div>
            <div class="kpi-sub">Decision trees</div></div>""", unsafe_allow_html=True)
    with i3:
        st.markdown(f"""<div class="kpi-card"><div class="kpi-icon">📐</div>
            <div class="kpi-title">Max Depth</div>
            <div class="kpi-value" style="color:#34d399;">{getattr(model,'max_depth','10')}</div>
