...
# Load datasets
try:
    candidate_profile = pd.read_csv('D:\\RP2\\Customer churn\\Candidate Profile.csv')
    call_log = pd.read_csv('D:\\RP2\\Customer churn\\Call log.csv')
    executive_profile = pd.read_csv('D:\\RP2\\Customer churn\\Executive Profile.csv')
...
for col in categorical_cols:
    if col in df_candidate.columns:
        df_candidate[col].fillna(df_candidate[col].mode()[0] if len(df_candidate[col].mode()) > 0 else 'Unknown', inplace=True)
...
df_candidate['Payment_Ratio'].fillna(0, inplace=True)
...
categorical_features = ['Source', 'Education', 'Background', 'Role', 'Current_status', 
                        ...
                        'Executive_Team', 'Executive_Location']
...
print("\n Final Regularized Model Selected for Deployment:")
final_model = regularized_models['RF (Regularized)']
...
print("""
 ...
 Best Model: Random Forest (Regularized)
 Final Performance:
   - Accuracy: {:.2f}%
   - F1 Score: {:.4f}
   - ROC-AUC: {:.4f}
